    ...
    return (
        <div className="flex flex-col items-center h-[100vh]">
            <div className="flex my-5 mb-10">
                ...
                {/*    Apply Crop*/}
                {/*</button>*/}
                <button onClick={() => flipCanvas(selectedPhoto, 'horizontal', canvas)} disabled={!selectedPhoto}
                        className="px-4 py-2 bg-blue-900 text-white disabled:opacity-20">
                    Flip Horizontal
                </button>
                <button onClick={() => flipCanvas(selectedPhoto, 'vertical', canvas)} disabled={!selectedPhoto}
                ...
                <button
                    onClick={() => enableCanvasCrop(isCropping, setIsCropping, canvas, croppedObject, setCroppedObject)}
                    disabled={!selectedPhoto} className="px-4 py-2 bg-green-800 text-white ml-2 disabled:opacity-20">
                    Crop Image
                </button>
                <button
                    onClick={() => applyCanvasCrop(croppedObject, croppedDimensions, canvas, setCroppedObject, setIsCropping)}
                    disabled={!selectedPhoto} className="px-4 py-2 bg-green-500 text-white ml-2 disabled:opacity-20">
                    Apply Crop
                </button>
            </div>
